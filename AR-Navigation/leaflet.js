window.addEventListener("load", () => {
    ...
    function waitForGraph() {
        if (
            ...
            // Create AR arrows function - modified to not depend on markers
            function createARArrows(navigationPoints, directionVectors) {
                console.log("Creating AR arrows for navigation points...");
                
                // Initialize AR arrow storage if not already done
                window.arNavigationArrows = window.arNavigationArrows || [];
                
                // If THREE.js scene is available, create the arrows
                if (window.THREE) {
                    ...
                    if (!window.arScene) {
                        window.arScene = new THREE.Scene();
                    }
                    
                    // Create a group to hold all arrows
                    const arrowsGroup = new THREE.Group();
                    window.arScene.add(arrowsGroup);
                    ...
                    console.warn("THREE.js not available. AR arrows could not be created.");
                    console.log("Navigation data is still available:", navigationPoints);
                }
                
                // Function to create an arrow pointing in the given direction
                function createArrow(angleDegrees) {
                    // If THREE.js is not available, return a placeholder
                    ...
                    arrowGroup.rotation.set(
                        Math.PI / 2, // Rotate 90 degrees around X to make arrow point forward
                        0,
            ...
            function drawPath(path) {
                ...
                // Create AR arrows at the 1-meter points along the path
